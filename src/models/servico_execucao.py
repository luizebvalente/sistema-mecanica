...
@servico_execucao_bp.route('/servicos-execucao/diagnostico', methods=['GET'])
def diagnostico_servicos():
    """Endpoint para diagnosticar problemas nos cálculos de tempo"""
    try:
        # Buscar todos os serviços em execução ou pausados
        ...
        for servico in servicos:
            try:
                # Tentar gerar o dict e identificar problemas
                data = servico.to_dict()
                
                # Verificar valores problemáticos
                campos_tempo = [
                    'tempo_decorrido_total_minutos',
                    'tempo_execucao_efetivo_minutos', 
                ...
                for campo in campos_tempo:
                    ...
                    if valor is None or math.isnan(float(valor)) if isinstance(valor, (int, float)) else False:
                        tem_problema = True
                        detalhes_problema.append(f"{campo}: {valor}")
                
                # Verificar strings formatadas
                campos_formatados = [
                    'tempo_decorrido_formatado',
                    'tempo_execucao_formatado',
                    'tempo_restante_formatado'
                ]
                
                for campo in campos_formatados:
                    valor = data.get(campo, '')
                    if 'NaN' in str(valor) or 'nan' in str(valor).lower():
                        tem_problema = True
                        detalhes_problema.append(f"{campo}: {valor}")
                ...
                if tem_problema:
                    problemas.append({
                        ...
                        'problemas': detalhes_problema,
                        'inicio': servico.inicio.isoformat() if servico.inicio else None,
                        'raw_data': {
                            'tempo_pausado_total': servico.tempo_pausado_total,
                            'tempo_extra_minutos': servico.tempo_extra_minutos,
                            'pausado_em': servico.pausado_em.isoformat() if servico.pausado_em else None
                        }
                    })
                else:
                    servicos_corrigidos.append({
                        'servico_id': servico.id,
                        'box_id': servico.box_id,
                        'cliente': servico.nome_cliente,
                        'status': servico.status,
                        'dados_calculados': {
                            'tempo_decorrido': data.get('tempo_decorrido_formatado'),
                            'percentual': data.get('percentual_conclusao'),
                            'tempo_restante': data.get('tempo_restante_formatado')
                        }
                    })
                ...
                problemas.append({
                    'servico_id': servico.id,
                    'box_id': getattr(servico, 'box_id', 'N/A'),
                    'cliente': getattr(servico, 'nome_cliente', 'N/A'),
                    'status': getattr(servico, 'status', 'N/A'),
                    'erro_critico': str(e),
        ...
        return jsonify({
            ...
            'servicos_com_problemas': len(problemas),
            'servicos_funcionando': len(servicos_corrigidos),
            'problemas_encontrados': problemas,
            'servicos_ok': servicos_corrigidos[:5],  # Limitar para não sobrecarregar
            'recomendacoes': [
                'Verificar se todos os serviços têm data de início válida',
                'Validar se tempo_pausado_total não está nulo',
                'Confirmar se tipo_servico tem tempo_estimado definido',
                'Aplicar correções no frontend para interceptar valores NaN'
            ]
        })
...
@servico_execucao_bp.route('/servicos-execucao/corrigir-dados', methods=['POST'])
def corrigir_dados_servicos():
    """Endpoint para corrigir dados problemáticos nos serviços"""
    try:
        # Buscar serviços com possíveis problemas
        servicos = ServicoExecucao.query.filter(
            ServicoExecucao.status.in_(['em_andamento', 'pausado'])
        ).all()
        ...
        for servico in servicos:
            ...
            # Correção 1: tempo_pausado_total nulo
            if servico.tempo_pausado_total is None:
                servico.tempo_pausado_total = 0
                correcoes_servico.append('tempo_pausado_total definido como 0')
            
            # Correção 2: tempo_extra_minutos nulo
            if servico.tempo_extra_minutos is None:
                servico.tempo_extra_minutos = 0
                correcoes_servico.append('tempo_extra_minutos definido como 0')
            
            # Correção 3: verificar se início é válido
            if not servico.inicio:
                # Se não tem início, definir como agora (caso crítico)
                servico.inicio = datetime.utcnow()
                correcoes_servico.append('data de início definida como agora')
            
            # Correção 4: recalcular fim_previsto se necessário
            if not servico.fim_previsto and servico.tipo_servico:
                tempo_total = (servico.tipo_servico.tempo_estimado or 60) + servico.tempo_extra_minutos
                servico.fim_previsto = servico.inicio + timedelta(minutes=tempo_total)
                correcoes_servico.append('fim_previsto recalculado')
            
            # Correção 5: validar status
            if servico.status not in ['em_andamento', 'pausado', 'concluido']:
                servico.status = 'em_andamento'
                correcoes_servico.append('status corrigido para em_andamento')
            
            # Correção 6: histórico de pausas
            if servico.historico_pausas is None:
                servico.historico_pausas = '[]'
                correcoes_servico.append('histórico de pausas inicializado')
            
            if correcoes_servico:
                correcoes_aplicadas.append({
                    'servico_id': servico.id,
                    'box_id': servico.box_id,
                    'cliente': servico.nome_cliente,
                    'correcoes': correcoes_servico
                })
        ...
        return jsonify({
            'timestamp': datetime.utcnow().isoformat(),
            'total_servicos_analisados': len(servicos),
            'servicos_corrigidos': len(correcoes_aplicadas),
            'correcoes_aplicadas': correcoes_aplicadas,
            'sucesso': True,
            'mensagem': f'{len(correcoes_aplicadas)} serviços foram corrigidos'
        })
...
@servico_execucao_bp.route('/servicos-execucao/status-dashboard', methods=['GET'])
def status_dashboard():
    """Endpoint específico para dashboard com dados seguros garantidos"""
    try:
        # Buscar serviços em execução
        servicos_execucao = ServicoExecucao.query.filter_by(status='em_andamento').all()
        
        # Buscar serviços pausados
        servicos_pausados = ServicoExecucao.query.filter_by(status='pausado').all()
        
        # Processar dados de forma ultra segura
        def processar_servico_seguro(servico):
            try:
                data = servico.to_dict()
                
                # Garantir que campos críticos nunca sejam NaN
                campos_criticos = [
                    'tempo_decorrido_total_minutos',
                    'tempo_execucao_efetivo_minutos',
                    'tempo_restante_minutos',
                ...
                # Garantir strings formatadas válidas
                campos_formatados = [
                    'tempo_decorrido_formatado',
                    'tempo_execucao_formatado', 
                    'tempo_restante_formatado'
                ...
                for campo in campos_formatados:
                    valor = data.get(campo, '0min')
                    if 'NaN' in str(valor) or valor is None:
                        data[campo] = '0min'
                
                return data
                
            except Exception as e:
                # Em caso de erro crítico, retornar estrutura mínima
                return {
                    'id': getattr(servico, 'id', 0),
                    'box_id': getattr(servico, 'box_id', 0),
                    'nome_cliente': getattr(servico, 'nome_cliente', 'Cliente'),
                    'status': getattr(servico, 'status', 'erro'),
                    'tempo_decorrido_formatado': '0min',
                    'percentual_conclusao': 0,
                    'erro_processamento': str(e)
                }
        
        dados_execucao = [processar_servico_seguro(s) for s in servicos_execucao]
        dados_pausados = [processar_servico_seguro(s) for s in servicos_pausados]
        
        return jsonify({
            'timestamp': datetime.utcnow().isoformat(),
            'servicos_em_execucao': dados_execucao,
            'servicos_pausados': dados_pausados,
            'estatisticas': {
                'total_execucao': len(dados_execucao),
                'total_pausados': len(dados_pausados),
                'boxes_ocupados': len(set(s['box_id'] for s in dados_execucao)),
                'boxes_com_pausados': len(set(s['box_id'] for s in dados_pausados))
            },
            'versao_api': 'ultra_segura_v2'
        })
        ...
        return jsonify({
            ...
            'estatisticas': {
                ...
                'total_pausados': 0,
                'boxes_ocupados': 0,
                'boxes_com_pausados': 0
            }
        }), 200  # Retornar 200 mesmo com erro para não quebrar frontend

# HELPER FUNCTION para uso em outros endpoints
def garantir_dados_seguros(servico_dict):
    """Função auxiliar para garantir que um dict de serviço não tenha valores NaN"""
    campos_numericos = [
        'tempo_decorrido_total_minutos',
        'tempo_execucao_efetivo_minutos',
        'tempo_restante_minutos',
    ...
    for campo in campos_numericos:
        if campo in servico_dict:
            valor = servico_dict[campo]
            if valor is None or (isinstance(valor, float) and math.isnan(valor)):
                servico_dict[campo] = 0
    
    campos_string = [
        'tempo_decorrido_formatado',
        'tempo_execucao_formatado',
        'tempo_restante_formatado',
        'tempo_estimado_formatado'
    ]
    
    for campo in campos_string:
        if campo in servico_dict:
            valor = servico_dict[campo]
            if valor is None or 'NaN' in str(valor):
                servico_dict[campo] = '0min'
