...
class ServicoExecucao(db.Model):
    ...
    def calcular_tempo_restante(self):
        """Calcula o tempo restante em minutos (pode ser negativo para atrasos)"""
        ...
        # Permitir valores negativos para indicar atraso
        return tempo_total_estimado - tempo_decorrido

    def calcular_tempo_real_atual(self):
        """Calcula o tempo real atual de execução em tempo real"""
        if not self.inicio:
            return {
        ...
        tempo_total_segundos = (agora - self.inicio).total_seconds()
        
        # Subtrair tempo pausado
        tempo_pausado_segundos = self.tempo_pausado_total * 60
        
        # Se está pausado atualmente, não contar o tempo desde a pausa
        if self.status == 'pausado' and self.pausado_em:
            tempo_pausa_atual = (agora - self.pausado_em).total_seconds()
            tempo_pausado_segundos += tempo_pausa_atual
        
        tempo_efetivo_segundos = max(0, tempo_total_segundos - tempo_pausado_segundos)
        
        # Calcular tempo total estimado
        tempo_total_estimado_segundos = (self.tipo_servico.tempo_estimado + self.tempo_extra_minutos) * 60 if self.tipo_servico else 3600
        
        # Calcular tempo restante (pode ser negativo)
        tempo_restante_segundos = tempo_total_estimado_segundos - tempo_efetivo_segundos
        
        # Verificar se está em atraso
        em_atraso = tempo_restante_segundos < 0
        atraso_segundos = abs(tempo_restante_segundos) if em_atraso else 0
        
        # Calcular percentual
        if tempo_total_estimado_segundos > 0:
            percentual = (tempo_efetivo_segundos / tempo_total_estimado_segundos) * 100
        else:
            percentual = 0
        
        return {
            'tempo_decorrido_segundos': int(tempo_efetivo_segundos),
            'tempo_restante_segundos': int(tempo_restante_segundos),
            'percentual_concluido': min(100, max(0, int(percentual))),
            'em_atraso': em_atraso,
