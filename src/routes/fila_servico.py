...
def get_fila_gestao():
    ...
    try:
        ...
        servicos_pausados = query_pausados.all()
        ...
        # Ordenar por box_id e depois por tipo (pausados primeiro, depois fila)
        resultado.sort(key=lambda x: (x['box_id'], 0 if x['tipo_item'] == 'pausado' else 1, x.get('posicao_fila', 0)))
